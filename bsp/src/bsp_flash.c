...
void SPI_Flash_WriteStruct( InfoStructTypedef *InfoStruct, u32_DataTypedef WriteAddr )
{
	u8 Buffer[18] = {0};
	...
	SPI_Flash_Write(Buffer,WriteAddr.u32_data,18);
	if( WriteAddr.u32_data < 18*7 )
		WriteAddr.u32_data += 18;
	else
		WriteAddr.u32_data = 18;
	SPI_Flash_Write(WriteAddr.arr_data, 0, 4);
}

void SPI_Flash_ReadStruct( InfoStructTypedef *InfoStruct, u32_DataTypedef ReadAddr )
{
	u8 Buffer[18];

	SPI_Flash_Read(Buffer,ReadAddr.u32_data,18);
	...
	SPI_Flash_Write(ReadAddr.arr_data, 4, 4);
}

void SPI_Flash_Get_WritePointer( RW_Ptr_Typedef *RW_Ptr_Struct )
{
...
void SPI_Flash_Get_ReadPointer( RW_Ptr_Typedef *RW_Ptr_Struct )
{
	u32_DataTypedef datatemp;
