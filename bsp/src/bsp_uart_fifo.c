...
#define USART3_DR_Base  0x40004804		// 0x40004800 + 0x04 = 0x40004804
...
static void DMA_USART2_Config(void);
static void DMA_USART3_Config(void);
static uint8_t UartGetBuff(UART_T *_pUart, uint8_t *_pBuff);

uint32_t g_CH2O_mg_m3;
...
*********************************************************************************************************
*/
uint8_t comGetBuff(COM_PORT_E _ucPort, uint8_t *_pBuff)
{
	UART_T *pUart;
	...
	return UartGetBuff(pUart, _pBuff);
}
#if 0
/*
*********************************************************************************************************
...
static void UartVarInit(void)
{
	...
	g_tUart1.usRxDMA_Channel = DMA1_Channel5;
#endif
	...
	g_tUart2.usRxDMA_Channel = DMA1_Channel6;
#endif
...
//	g_tUart3.ReciveNew = RS485_ReciveNew;		/* 接收到新数据后的回调函数 */
	...
	g_tUart3.usRxDMA_Channel = DMA1_Channel3;
#endif
...
static void DMA_USART1_Config(void)
{
	...
	/*内存地址(要传输的变量的指针)*/
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)g_tUart1.pRxBuf;
	
	/*方向：从外设到内存*/		
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;//DMA_DIR_PeripheralDST;	

	/*传输大小DMA_BufferSize=UART1_RX_BUF_SIZE*/	
...
static void DMA_USART2_Config(void)
{
	...
	/*内存地址(要传输的变量的指针)*/
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)g_tUart2.pRxBuf;
...
//	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;	
	...
	/*传输大小DMA_BufferSize=UART2_RX_BUF_SIZE*/	
...
static void DMA_USART3_Config(void)
{
	...
	/*内存地址(要传输的变量的指针)*/
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)g_tUart3.pRxBuf;
...
static uint8_t UartGetBuff(UART_T *_pUart, uint8_t *_pBuff)
{
	...
	{
		//*_pBuff = _pUart->pRxBuf[_pUart->usRxRead];		/* 从串口接收FIFO取1个数据 */
		memcpy(_pBuff,_pUart->pRxBuf,usCount+1);
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if( USART_GetITStatus( _pUart->uart, USART_IT_IDLE ) == SET )
	{
		//LED1_ON;
		DMA_Cmd( _pUart->usRxDMA_Channel, DISABLE );
		
		if( _pUart->usRxDMA_Channel == DMA1_Channel5 )
			DMA_ClearFlag(DMA1_FLAG_TC5);
		else if( _pUart->usRxDMA_Channel == DMA1_Channel6 )
			DMA_ClearFlag(DMA1_FLAG_TC6);
		else if( _pUart->usRxDMA_Channel == DMA1_Channel3 )
			DMA_ClearFlag(DMA1_FLAG_TC3);
		_pUart->usRxCount = _pUart->usRxBufSize - _pUart->usRxDMA_Channel->CNDTR;
		_pUart->pRxBuf[ _pUart->usRxCount ] = '\0';
		_pUart->usRxDMA_Channel->CNDTR = _pUart->usRxBufSize;
		DMA_Cmd( _pUart->usRxDMA_Channel, ENABLE );
		//清除空闲线中断标志位
		USART_ReceiveData( _pUart->uart );
		if( _pUart->uart == USART2 || _pUart->uart == USART3 )
			USART_CallBack();
