...
#endif
{
	...
	if (TIM_GetITStatus(TIM_HARD, TIM_IT_Update))
	{
		TimingDelay_Decrement();
		TIM_ClearITPendingBit(TIM_HARD , TIM_FLAG_Update);
		TIM_ITConfig(TIM_HARD,TIM_IT_Update,DISABLE);
	}
}
...
//	}
//	/*
//	u32 ticks = _uiTimeOut * SYSCLK_FREQ_72MHz / 1000000;
...
__asm void Delayus(u32 usec,u8 freq)  //freq参数为系统时钟频率（SYSCLK），单位MHz，且必须能被4整除，且必须大    于等于16,常用的
